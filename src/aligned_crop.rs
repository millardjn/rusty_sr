...
pub trait AlignedCrop: DataSet {
	fn aligned_crop(self, component: usize, shape: &[usize], cropping: Cropping) -> AlignedCropSet<Self>
	where
		Self: Sized,
	{
...
impl<S: DataSet> DataSet for AlignedCropSet<S> {
	fn get(&mut self, i: usize) -> Vec<ArrayD<f32>> {
		...
		for &(other_component, ref factors) in self.other_crops.iter() {
			...
			let (next_crop_shape, next_input_slice_arg, next_output_slice_arg) =
...
fn crop(
	...
	crop_shape: &[usize],
	fill: f32,
	input_slice_arg: SmallVec<[SliceOrIndex; 6]>,
	output_slice_arg: SmallVec<[SliceOrIndex; 6]>,
) -> ArrayD<f32> {
...
fn slice_args(
	...
	crop_shape: &[usize],
	cropping: &Cropping,
) -> (SmallVec<[SliceOrIndex; 6]>, SmallVec<[SliceOrIndex; 6]>) {
